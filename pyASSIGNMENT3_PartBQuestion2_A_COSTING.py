...
#SOLUTION WILL DETERMINE THE PATH FOR THE MINIMUM COST
...
def shortest_path(graph, source, destination):
    ...
    while previous_vertex is not None and previous_vertex is not source:
        ...
        timeTravel1 = int(timeTravel1) + int(distanceCal(previous_vertex, source))/int(crocspeed)
        timeTravel2 = int(timeTravel2) + int(distanceCal(previous_vertex, source))/80 #80 kms per hour
...
#                                'Notes': 'There is no path '}, ignore_index=True)
        ...
        if boolVerbose==True:
            ...
            print("crocDensity for this path   is: ", densityCroc)
    ...
    crocDF=crocDF.append( {'StartNode': source,
                           ...
                           'Cost': costCalc,
                           'crocDensity':densityCroc}, ignore_index=True)
...
def Compute_Costing(intNodeA, intNodeB):
    shortest_path(g, intNodeA, intNodeB)
    print('Search Path KPIs')
    MinCost = crocDF[crocDF.Cost == crocDF.Cost.min()]
    print("---MIN COST--")
    print(MinCost.to_string())

    #brute force saerch ON 32 NODES
def Compute_Costing_Brute_Force():
    global crocDF
    countA = 32
    countB = 32
    while countA > 0 :
...
Compute_Costing(30,29)
#Exhaustive Search implementation
Compute_Costing_Brute_Force()