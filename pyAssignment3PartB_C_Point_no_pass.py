...
def Point_no_Pass(nodeA, nodeB):
    shortest_path(g, nodeA, nodeB)
    densitySightingsDistanceQuery = crocDF[crocDF.crocDensity == crocDF.crocDensity.max()]
    print("---CROC DENSITY  MAX---")
    print(densitySightingsDistanceQuery)



    #brute force saerch ON 32 NODES
def Point_no_Pass_Brute_Force():
    countA = 32
    countB = 32
    while countA > 0 :
        countB = 32
    ...
    densitySightingsDistanceQuery = crocDF[crocDF.crocDensity == crocDF.crocDensity.max()]
    print("---CROC DENSITY  MAX---")
    print(densitySightingsDistanceQuery.to_string())

#single Search implementation
Point_no_Pass(1,32)
#exhaustive Search implementation
Point_no_Pass_Brute_Force()