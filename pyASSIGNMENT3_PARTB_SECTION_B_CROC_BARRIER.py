...
crocDF = pd.DataFrame(columns=['StartNode',
                               'EndNOde',
                               'Path',
                               'Sightings',
                               'Distance',
...
def shortest_path(graph, source, destination):
    ...
    while previous_vertex is not None and previous_vertex is not source:
        ...
        #if on land sum sightings
        if locationList[previous_vertex, 1, 1]==0:
            CrocLandTravelCalc=int(CrocLandTravelCalc)+int(currentSightingValue)


        currentDistanceForThisNode=int(distanceCal(previous_vertex, source))
        sightingsCount=int(sightingsCount)+ int(currentSightingValue)
        if boolVerbose==True:
...
#                                'Notes': 'There is no path '}, ignore_index=True)
    ...
    crocDF=crocDF.append( {'StartNode': source,
                           'EndNOde': destination,
                           'Path': path,
                           'Sightings': sightingsCount,
                           'Distance': distanceCalc,
                           'CrocSpeed': timeTravel1,
                           'RangerSpeed': timeTravel2,
                           'RevisitedNodes': NodesVisited,
                           'Notes': speacialComments,
                           'cost': costCalc,
                           'crocDensity':densityCroc,
                           'LandTravelCalc':int(CrocLandTravelCalc)},
                            ignore_index=True)
...
for index, row in df.iterrows():
    ...
    #print(" Adding node " + str(nodeId) + "    sightings:" + str(sightings) + "    enviro:" +str(enviro))
...
def Crock_Blocker(nodeA, nodeB):
    shortest_path(g, nodeA, nodeB)


    crocDF.sort_values(by=['LandTravelCalc'], ascending=False)
    print("---CROC LAND PATH  MAX---")
    print(crocDF.to_string())



    #brute force saerch ON 32 NODES
def Crock_Blocker_Brute_Force():
    countA = 32
    countB = 32
    while countA > 0 :
        countB = 32
