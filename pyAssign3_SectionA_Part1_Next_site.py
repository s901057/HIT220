...
def Next_Site(intA, intB):
    shortest_path(g, intA, intB)
    maxySightingsDistanceQuery = crocDF[crocDF.Sightings == crocDF.Sightings.max()]
    print("--- MAX SIGHTINGS   Path---")
    print(maxySightingsDistanceQuery.to_string())


    #brute force saerch ON 32 NODES
def Next_Site_Brute_Force():
    countA = 32
    countB = 32
    while countA > 0 :
        countB = 32
...
#single Search implementation for a specific path!
Next_Site(1,32)

#exhaustive Search implementation of all combinations!
Next_Site_Brute_Force()